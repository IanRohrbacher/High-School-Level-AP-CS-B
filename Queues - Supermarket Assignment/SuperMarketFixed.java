			...
			simulation: while (minutes < runningMinutes) { // 720
				...
				if (line.size() > 0) {
					...
					System.out.println("Customer in the Front Checking out Percent : "
							+ line.peek().getWaitedTime() + "/" + line.peek().getServiceTime());

					// one minute happens so they can leave, move along in line or keep checking out
					if (line.peek().isLeaving()) {
						// if the person is leaving they get removed and longest wait is recorded
						int timeWated = minutes - line.peek().getTimeWhenEntered();
						longestWait = (timeWated > longestWait) ? timeWated : longestWait;
						System.out.println("Longest Waited Time: " + longestWait);
						System.out.println("Customer in the Front is Leaving and Waited : " + timeWated);
						line.remove();
					}
				}
				...
				if (newCustomerTime == newCustomer) {
					Customer person = new Customer((int) ((Math.random() * 4) + 1), minutes);
					line.add(person);
